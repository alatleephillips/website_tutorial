...
-   👩‍🔬 \| My [personal science website](www.alexandraatleephillips.com), built from the [story template](https://html5up.net/story)

-   🏢 \| A [science business website](www.mirios.net), built from the [hyperspace template](https://html5up.net/hyperspace)

-   🧬 \| A [science nonprofit website](www.womendoingscience.com), built from the (paid) [indivisible template](https://pixelarity.com/indivisible)
...
4.  You can type `git status` to see if changes need to be pushed to Github
5.  Then to commit them all type `git add .` followed by `git commit -m "any message here"`
6.  Finally push with `git push`
...
**Where should I host my website?** Right now your website only exists locally, as that `index.html` file - but we want to share it with the world! For hosting, I prefer to use [Netlify](https://www.netlify.com/) because their base plan is ✨ free ✨and they integrate really well with Github.

1.  Sign up for an account, using your Github log in.
...
Now look further down for something like `<ul class="actions special">` (with more code in between) `</ul>` - add `</form>` after that. Finally, head to Netlify and go to site settings, then forms. Scroll to outgoing notifications and click to set up email notifications. Should be all set!

::: callout-tip
It's always a good idea to have a friend test out a new form for you to make sure it's working!
:::
...
Ready to make a change to the colors or font? Time to dive into the CSS code! Going into all of these details is beyond this tutorial, but hopefully
...
-   Tips
    ...
    -   Make sure any forms you create are enabled to send email notifications to you when they get filled out

    -   Enable "pretty URLS" on netlify to avoid your webpages ending in .html - to do this go to your site - then site settings - build and deploy - scroll to processing - asset optimization - edit settings - enable - check box for pretty URLs
